...
export default function ManufacturingInitiative({
  ...
  const [guardrailFailureData, setGuardrailFailureData] = useState<{
   ...
   const CardTitle = 'Manufacturing Agency';
...
//   }, [location.pathname, setRunTravel, setRunManufacturing, setRunInitiative]);
 ...
 const responseData = await response.json();
  ...
  return (
    <div className={`manufacturing-run-container ${interactHeight ? 'h-100' : 'h-100'}`}>
...
const LandingPage = ({
  ...
  return (
    <div className="d-flex flex-column align-items-center text-center justify-content-center h-100">
      ...
      <h2
        className={`text-white ${source === RunInitiativeSource.Interact && !isExpanded ? '' : 'medium-weight-margined'}`}>
        {' '}
        Welcome to Manufacturing Agency!{' '}
      </h2>
      <p
        className={`text-white semi-thick text-center justify-content-center align-items-center mx-auto mt-2 ${source === RunInitiativeSource.Interact && !isExpanded ? '' : ' wide-margined'}`}>
